...
#define DIRECTORY_SHADERS "shader/"
...
void Shader::init(const GLchar *filename)
{
	...
	/*
	printf("Path relativo: %s\n", tempFullFileName.c_str());
	printf("%s\n", std::filesystem::absolute(tempFullFileName).string().c_str());
	*/
	...
	free(buffer);
	std::cout << "Shader " << filename << " loaded successful" << std::endl;
	mAllOK = true;
}
...
GLuint Shader::getID() {
	if (mIDprogram == 0) {
		std::cout << "Warning: pasando IDprogram null" << std::endl;
	}
	return mIDprogram;
}

void Shader::Use() {
	if (mAllOK) glUseProgram(mIDprogram);
	else std::cout << "ERROR: shader no iniciado" << std::endl;
}
