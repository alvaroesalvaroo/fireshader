...
void TexturedLitPlane::initKnownShader() {
    ...
    mShader->Use();
    printf("Lit Textured cube or plane: program id %d inited from shader name %s\n", mShader->getID(), mShaderName);

    mLightColorUniform = glGetUniformLocation(mShader->getID(), "lightColor");
    printf("Debug uniform lightcolor %d\n", mLightColorUniform);
    glUniform3f(mLightColorUniform, mLightColor.x, mLightColor.y, mLightColor.z);

    /* // If we want to add color to the already textured object
    mObjectColorUniform = glGetUniformLocation(mShader->getID(), "objectColor");
    printf("Debug uniform objectcolor %d\n", mObjectColorUniform);
    ...
    */
    ...
    printf("Debug uniform lightposition %d\n", mLightPositionUniform);
    ...
    printf("Debug uniform viewposition %d\n", mViewPositionUniform);
    printf("%s shader inited with assigned unifrms lighcolor, lighposition and lightcolor \n", mShaderName);
...
void TexturedLitPlane::generateCube(float size) {
    ...
    // VAMOS A DEJARLO EN UN PLANO DE MOMENTO
...
void TexturedLitPlane::prepareVAO() {
    printf("preparing all vaos for textured lit plane/cube \n");
    ...
    // 3 ATRIBUTES: POSICION, NORMALS UV y STRIDE 8
    ...
    glBindVertexArray(0); // pODRIA dar esto problemas?
