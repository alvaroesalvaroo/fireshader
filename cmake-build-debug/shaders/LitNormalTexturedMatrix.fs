...
// UNIFORMS
uniform sampler2D textura;          // Main texture
...
const float materialAmbient = 0.15;
...
vec3 CalcPointLight(PointLight light, vec3 normal, vec3 fragPos, vec3 viewDir)
{
    ...
    // combine results
    vec3 ambient  = light.ambient  * vec3(texture(textura, FragTexture));
    vec3 diffuse  = light.diffuse  * diff * vec3(texture(textura, FragTexture));
    vec3 specular = light.specular * spec * vec3(texture(textura, FragTexture));
...
void main() {
    ...
    // Two parameters we need before calculating light influence
	//vec3 norm = normalize(Normal);
	vec3 norm = normalize(texture(normalTexture, FragTexture).rgb);

	vec3 viewDirection = normalize(viewPosition - FragPosition);

	// Light influence value
    vec3 result = CalcPointLight(pointLight, norm, FragPosition, viewDirection);
